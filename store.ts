...
export function BarberProvider({ children }: { children?: ReactNode }) {
  ...
  const login = async (id: string, pass: string) => {
    ...
    if (id === 'hermanosadm@gmail.com' && pass === adminPass) {
      const adminName = config.adminName || 'Hermanos';
  ...
  const asaasRequest = async (endpoint: string, method = 'GET', body?: any) => {
    ...
    const res = await fetch('https://us-central1-hermanos-90985.cloudfunctions.net/asaasProxy', {
      method: 'POST',
      ...
      try {
        ...
        if (customerId) {
          ...
          const charge = await asaasRequest('/payments', 'POST', {
            ...
            description: `Hermanos Barbershop — ${appt.serviceName}`,
      ...
      try {
        ...
        if (customerId) {
          ...
          const sub = await proxy('/subscriptions', 'POST', {
            ...
            cycle,
            description:       `${data.planName} — Hermanos Barbershop`,
